...
def display_app_title():
    ...
    タイトル表示（メイン画面に表示）
    """
    st.title(ct.APP_NAME)  # ← タイトルを中央に大きく表示
...
def display_select_mode():
    ...
    with st.sidebar:
        ...
        st.info("入力内容と関連性が高い社内文書のありかを検索できます。", icon="📄")  # ← 青背景で説明
        st.code("【入力例】\n社員の育成方針に関するMTGの議事録", wrap_lines=True, language=None)

        st.subheader("「社内問い合わせ」を選択した場合")
        st.info("質問・要望に対して、社内文書の情報をもとに回答を得られます。", icon="💬")  # ← 青背景で説明
            ...
            # LLMからの回答の場合
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        # ① メインドキュメントのページ番号がある場合に表示に追加
                        if "main_page_number" in message["content"]:
                            st.success(f"{message['content']['main_file_path']}（ページNo.{message['content']['main_page_number']+1}）", icon=icon)  # ← 修正箇所
                        else:
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            # サブドキュメントのページ番号がある場合に表示に追加
                            for sub_choice in message["content"]["sub_choices"]:
                                icon = utils.get_source_icon(sub_choice['source'])
                                if "page_number" in sub_choice:
                                    st.info(f"{sub_choice['source']}（ページNo.{sub_choice['page_number']+1}）", icon=icon)  # ← 修正箇所
                                else:
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        if "page" in llm_response["context"][0].metadata:
            ...
            st.success(f"{main_file_path}（ページNo.{main_page_number+1}）", icon=icon)  # ← 修正箇所
        else:
        ...
        for document in llm_response["context"][1:]:
            ...
            if "page" in document.metadata:
                ...
                sub_choice = {"source": sub_file_path, "page_number": sub_page_number}  # ← 修正箇所
        ...
        if sub_choices:
            ...
            for sub_choice in sub_choices:
                ...
                if "page_number" in sub_choice:
                    ...
                    st.info(f"{sub_choice['source']}（ページNo.{sub_choice['page_number'] + 1}）", icon=icon)  # ← +1 補正を追加
                else:
                    # 「サブドキュメントのファイルパス」を表示
                    st.info(f"{sub_choice['source']}", icon=icon)
...
def display_contact_llm_response(llm_response):
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            # ページ番号が取得できた場合のみ、ページ番号を表示（ドキュメントによっては取得できない場合がある）
            if "page" in document.metadata and file_path.lower().endswith(".pdf"):  # ← 修正箇所
                page_number = document.metadata["page"]
                file_info = f"{file_path}（ページNo.{page_number+1}）"  # ← 修正箇所
            else:
